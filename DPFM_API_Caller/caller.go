...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var partnerFunctionData *dpfm_api_output_formatter.PartnerFunction
	for _, a := range accepter {
		switch a {
		case "PartnerFunction":
			h := c.partnerFunctionDelete(input, output, log)
			partnerFunctionData = h
			if h == nil {
				continue
			}
		}
	...
	return &dpfm_api_output_formatter.Message{
		PartnerFunction: partnerFunctionData,
	}
}

func (c *DPFMAPICaller) partnerFunctionyDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.PartnerFunction {
	sessionID := input.RuntimeSessionID
	partnerFunction := c.PartnerFunctionRead(input, log)
	partnerFunction.IsMarkedForDeletion = input.PartnerFunction.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": partnerFunction, "function": "PartnerFunctionPartnerFunction", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "PartnerFunction Data cannot delete"
		return nil
	}
